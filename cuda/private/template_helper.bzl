...
def _generate_toolchain_build(repository_ctx, cuda):
    tpl_label = Label("//cuda/private:templates/BUILD.toolchain_nvcc")
    local_platform = "linux" if _is_linux(repository_ctx) else "windows"
    ...
    substitutions = {
        ...
        "%{nvcc_local_actual}": "nvcc-{}-toolchain".format(local_platform),
        # Local installations only contain tools for the client's OS.
        "%{nvcc_platforms}": "linux,windows" if cuda.path == None else local_platform,
